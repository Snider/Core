...
func (m *MockWorkspaceService) CreateWorkspace(identifier, password string) (string, error) {
	return "", nil
}
func (m *MockWorkspaceService) SwitchWorkspace(name string) error { return nil }
func (m *MockWorkspaceService) WorkspaceFileGet(filename string) (string, error) {
	return "", nil
}
func (m *MockWorkspaceService) WorkspaceFileSet(filename, content string) error { return nil }

type MockCryptService struct{}

func (m *MockCryptService) GenerateKey(opts ...Option) (any, error) { return nil, nil }
func (m *MockCryptService) Encrypt(pub any, data []byte) ([]byte, error)   { return nil, nil }
func (m *MockCryptService) Decrypt(priv any, data []byte) ([]byte, error)  { return nil, nil }
func (m *MockCryptService) Sign(priv any, data []byte) ([]byte, error)     { return nil, nil }
func (m *MockCryptService) Verify(pub any, data, sig []byte) error         { return nil }
...
func TestCore_Services_Good(t *testing.T) {
	...
	err = c.RegisterService("workspace", &MockWorkspaceService{})
	assert.NoError(t, err)

	err = c.RegisterService("crypt", &MockCryptService{})
	assert.NoError(t, err)
